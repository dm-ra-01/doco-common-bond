...
| `match-backend` / `planner-backend` | Python 3.12, FastAPI, OR-Tools                     | Containerised allocation microservice. Receives allocation jobs from the orchestrator Edge Function, runs the CP-SAT constraint solver, writes results back to Postgres atomically via RPC.                     |
