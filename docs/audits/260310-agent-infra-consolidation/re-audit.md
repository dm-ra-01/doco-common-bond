...
| **ARCH-01-T4** | Create symlinks for all 9 repos → category dirs                  | ✅ Done | `readlink` verified for all: planner/preference/workforce/website → `frontend-nextjs`; supabase-receptor → `supabase-infrastructure`; match-backend/receptor-planner → `backend-python`; common-bond → `docusaurus`; rotator_worker → `dart-flutter` |
...
| **RP-01**   | Delete `receptor-planner/.agents/skills/python-*`                                              | ✅ Done | Committed on `audit/260310-agent-infra-consolidation`                                             |
...
| Python backends (match-backend, receptor-planner)           | `pytest`                                              | **Not re-run** — all changes were file deletions in `.agents/` and `.gitignore` additions. No Python source modified.                                                                    |
