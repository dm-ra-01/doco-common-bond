...
| Control ID | Control Name                                               | Applicable | Justification                                                                               | Implementation Status                                                                                                                        |
| ---------- | ---------------------------------------------------------- | ---------- | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| 6.1        | Screening                                                  | Yes        | Pre-employment screening relevant to all staff with access to PII or systems                | Partial — ⚠️ Confirm background check policy for Territory Managers                                                                          |
| 6.2        | Terms and conditions of employment                         | Yes        | Employment contracts must reference information security obligations                        | Partial — ⚠️ Confirm whether current employment contracts for Amelia Jane Cameron and Emma Nyhof include information security terms          |
...
| 6.6        | Confidentiality or non-disclosure agreements               | Yes        | NDA obligations relevant to staff with access to client and product information             | ⚠️ Confirm — confirm whether Territory Manager employment contracts include confidentiality clauses                                          |
...
| 7.10       | Storage media                                         | Yes        | Portable storage media (USB drives) poses data exfiltration risk                                                                      | Partial — ⚠️ Confirm whether USB storage is permitted or restricted under current policy                        |
...
| Control ID | Control Name                                                | Applicable | Justification                                                                                                          | Implementation Status                                                                                                                |
| ---------- | ----------------------------------------------------------- | ---------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| 8.1        | User endpoint devices                                       | Yes        | Developer laptops are the primary endpoint devices; PII accessed from these devices                                    | Partial — encryption and MFA enforced; formal endpoint management policy (MDM) not yet implemented                                   |
...
| 8.10       | Information deletion                                        | Yes        | PII must be deleted in accordance with Privacy Act obligations and user requests                                       | Planned — no formal data deletion procedure documented; ⚠️ Confirm whether deletion requests can currently be processed end-to-end   |
...
| 8.23       | Web filtering                                               | Yes        | Applicable to restrict access to malicious sites from company devices                                                  | Partial — ⚠️ Confirm whether Cloudflare Gateway or equivalent web filtering is enabled for company devices                           |
...
| 8.30       | Outsourced development                                      | No         | All development is performed in-house by Common Bond staff. No external development contractors are currently engaged. | ⚠️ Confirm — if contractors are engaged in future, this control becomes applicable                                                   |
...
| 8.33       | Test information                                            | Yes        | Production PII must not be used in test environments                                                                   | Partial — ⚠️ Confirm whether production data is ever copied to development environments                                              |
