...
| 8.16       | Monitoring activities                                       | Yes        | Monitoring of systems for security events is required                                                                  | Implemented — Prometheus + Grafana + Alertmanager + Loki deployed on k3s cluster (`monitoring` namespace). Alert channel: Slack only (#incidents for P1/P2, #deployments for informational — no email/PagerDuty, per PROC-02 clarification). Log retention: 30-day online Loki, archived to Cloudflare R2 APAC monthly. See [kubernetes-cluster.md](../../infrastructure/environment/kubernetes-cluster.md) §Monitoring Stack and [PROC-02 Incident Response](./incident-response). Implemented via audit 260312-cicd-environments (ARCH-07-T2, 2026-03-12). |
